...
{
	my $upsBatteryStatus = ".1.3.6.1.4.1.33264.1.1.2.1.6.1.1.3.3";
	my $upsOutputSource = "1.3.6.1.2.1.33.1.4.1.0";
    my $upsMainsStatus = "1.3.6.1.4.1.33264.1.1.2.1.6.1.1.3.1";
    ...
    $upsMainsStatus = $result->{$upsMainsStatus};
	if (($upsOutputSource !~ /^noSuch/) &&
		($upsBatteryStatus !~ /^noSuch/) &&
        ($upsMainsStatus !~ /^noSuch/)) {
            $output = "UPS State is ".$upsOutputSources{$upsOutputSource}.". Battery State is ".$upsBatteryStatuses{$upsBatteryStatus}.". Mains Power State is ".$upsMainsStatuses{$upsMainsStatus}.".";

		if (($upsOutputSource == 3) && ($upsMainsStatus =0)) { # normal
			$state = 'OK';
